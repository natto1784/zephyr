...
static ALWAYS_INLINE bool ti_eqep_is_late(const struct device *dev, uint32_t ticks, uint32_t now)
{
	...
	uint32_t top = DEV_REGS(dev)->QPOSMAX;

	if (ti_eqep_is_counting_up(dev)) {
		return (ticks > now && ticks <= (now + guard) % top);
	}

	return (ticks < now && ticks >= (now + top - guard) % top);
}
...
void z_impl_ti_eqep_configure_capture(const struct device *dev,
				      const struct ti_eqep_cap_cfg *cap_cfg)
{
	struct ti_eqep_regs *regs = DEV_REGS(dev);

	/* disable capture unit before configuration */
	regs->QCAPCTL &= ~TI_EQEP_QCAPCTL_CEN;
					      ...
					      uint32_t *period, bool scale)
{
	...
	if (!(qcapctl & TI_EQEP_QCAPCTL_CEN)) {
		LOG_ERR("Capture unit is disabled");
		return -ENOTSUP;
	}
...
#define TI_EQEP_DEFINE_CLK_SUBSYS(n)                                                               \
...
static void z_vrfy_ti_eqep_configure_capture(const struct device *dev,
					     const struct ti_eqep_cap_cfg *cap_cfg)
{
	...
	z_impl_ti_eqep_configure_capture(dev, (const struct ti_eqep_cap_cfg *)&cfg_copy);
}
#include <zephyr/syscalls/ti_eqep_configure_capture_mrsh.c>

static void z_vrfy_ti_eqep_configure_qep(const struct device *dev,
...
static void z_vrfy_ti_eqep_configure_decoder(const struct device *dev,
					     const struct ti_eqep_dec_cfg *dec_cfg)
{
						     ...
						     uint32_t *period, bool scale)
{
	...
	return z_impl_ti_eqep_get_latched_capture_values(dev, timer, period, scale);
}
