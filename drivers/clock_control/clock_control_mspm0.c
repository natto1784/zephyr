...
#define MSPM0_ULPCLK_IS_MCLK_DIV_4  1
#define MSPM0_MCLK_CANNOT_USE_LFCLK 1
#endif

#if DT_NODE_HAS_PROP(DT_NODELABEL(ckm), cpuss)
...
static int clock_mspm0_configure_lfclk(const struct device *sysctl, enum mspm0_clock_source source)
{
	...
	case MSPM0_CLOCK_SRC_LFOSC:
		...
		if (mux != SYSCTL_CLKSTATUS_LFCLKMUX_VAL_LFOSC) {
			LOG_ERR("LFCLK cannot be switched back to LFOSC once moved off it; do a "
				"BOR/POR to enable LFOSC as source");
			return -EIO;
		}

		/* a NRST (>1s) can reset LFCLKMUX to its default (LFOSC) value without
		 * re-enabling LFOSC if it was previously disabled by a switch away from
		 * it -- only a BOR/POR clears that latch. Wait for LFOSCGOOD to tell the
		 * two cases apart.
		 */
		ret = clock_mspm0_wait_clkstatus(sysctl, SYSCTL_CLKSTATUS_LFOSCGOOD, true,
						 MSPM0_CLK_WAIT_TIMEOUT_US);
		if (ret == -ETIMEDOUT) {
			LOG_ERR("LFOSC not ready; if LFCLK was previously moved off LFOSC, "
				"only a BOR/POR (not NRST) can re-enable it");
		}
		if (ret < 0) {
			return ret;
		}
...
#else
	ret = clock_mspm0_configure_lfclk(cfg->sysctl, MSPM0_CLOCK_SRC_LFOSC);
	if (ret < 0) {
		LOG_WRN("LFOSC not available, continuing without it: %d", ret);
		ret = 0;
	}
#endif
	if (ret < 0) {
