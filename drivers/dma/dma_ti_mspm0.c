...
static inline int dma_ti_mspm0_get_addr_incr(uint8_t adj, uint32_t *incr)
{
	if (incr == NULL) {
		return -EINVAL;
	}
	...
	case DMA_ADDR_ADJ_INCREMENT:
		*incr = DMA_MSPM0_INCR_INCREMENT;
		break;
	case DMA_ADDR_ADJ_NO_CHANGE:
		*incr = DMA_MSPM0_INCR_UNCHANGED;
		break;
	case DMA_ADDR_ADJ_DECREMENT:
		*incr = DMA_MSPM0_INCR_DECREMENT;
		break;
	default:
		return -EINVAL;
	}
					       ...
					       uint16_t interval, uint8_t addr_adj, uint32_t *incr)
{
	...
	return dma_ti_mspm0_get_addr_incr(addr_adj, incr);
}
				  ...
				  struct dma_config *config)
{
	...
	if (config->half_complete_callback_en) {
		if (channel < 8 && dma_ti_mspm0_is_full_channel(cfg, channel)) {
	...
	K_SPINLOCK(&dma_data->lock) {
		...
		cfg->regs->cpu_int.imask |= BIT(channel);
		if (config->half_complete_callback_en && channel < 8) {
